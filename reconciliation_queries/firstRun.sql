INSERT INTO `mdp`.`reconciliation_pivot_2` (local_value, external_value)
SELECT localId, externalId  FROM (SELECT 
...
LEFT JOIN
    `reconciliation_external_values_2` ON 
        ...
        AND `localValues`.`referencia_1` = `reconciliation_external_values_2`.`referencia_1` 
         AND ( `localValues`.`referencia_1` = `reconciliation_external_values_2`.`referencia_1` 
         OR `localValues`.`referencia_1` = `reconciliation_external_values_2`.`referencia_2` 
         OR `localValues`.`referencia_1` = `reconciliation_external_values_2`.`referencia_3` )
        
WHERE
    `localValues`.`local_account`  IN ('11100501')
    AND `localValues`.`valor_debito` > 0
    AND `reconciliation_external_values_2`.`local_account` IS NOT NULL
	AND `localValues`.`fecha_movimiento` <= '2023-09-08') AS firstRun